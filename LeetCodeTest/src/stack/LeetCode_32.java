...
public class LeetCode_32 {
    ...
    /*
    双指针双向扫描方法概述，两个指针left，right同时从数组左侧向右遍历，遇到"("则left加一，遇到")"则right加一，当left=right时则说明遇到了一对正确输入的括号，result++，如果右括号比左括号数量大，即出现了())类似的情况，说明出现不正确输入，记录当前字符串长度值，将left和right置零并继续向后遍历。但是当前若出现(()，则left永远不等于right，需要保留result值再从右至左遍历一遍就可解决这种情况。
     */
